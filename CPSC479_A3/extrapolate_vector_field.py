...
class Extrapolator:
    ...
    def extrapolate(self, x, y, z1, z2, out_size):
        ...
        x_lin = np.linspace(0, out_size[1] - 1, out_size[1])
        y_lin = np.linspace(0, out_size[0] - 1, out_size[0])
        grid_x, grid_y = np.meshgrid(x_lin, y_lin)
        
        grid_points = np.column_stack((grid_x.flatten(), grid_y.flatten()))

        z1_out = interpolate.griddata((x, y), z1, grid_points, method='linear', fill_value=0)
        z2_out = interpolate.griddata((x, y), z2, grid_points, method='linear', fill_value=0)

        z1_out = z1_out.reshape(grid_x.shape)
        z2_out = z2_out.reshape(grid_y.shape)
